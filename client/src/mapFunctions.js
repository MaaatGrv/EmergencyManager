...
// Don't initialize the map here
let mymap;
let fireMarkers;
let facilityMarkers;
let vehicleMarkers;

let currentFireMarkers = new Map();
let currentVehicleMarkers = new Map();

export const initMap = () => {
    // Only initialize the map if it's not already initialized
    if (!mymap || !mymap._container) {
        mymap = L.map('mapid').setView([45.75647670543468, 4.866953950030793], 13);
        fireMarkers = L.layerGroup().addTo(mymap);
        facilityMarkers = L.layerGroup().addTo(mymap);
        vehicleMarkers = L.layerGroup().addTo(mymap);
...
export const displayFires = async () => {
...
export const displayVehicles = async () => {
